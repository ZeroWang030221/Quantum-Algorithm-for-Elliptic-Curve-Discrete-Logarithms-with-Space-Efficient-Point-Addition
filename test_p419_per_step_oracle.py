"""Exhaustive per-microstep oracle comparison for p=419.
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--out',default='p419_per_step_oracle.json');ap.add_argument('--verbose',action='store_true');ap.add_argument('--require-real-qiskit',action='store_true');a=ap.parse_args()
    if a.require_real_qiskit and not USING_REAL_QISKIT: raise SystemExit('real Qiskit is required but is not installed')
    p=P;n=p.bit_length();cases=list(range(1,p));cfg=eea.get_n_config(n);Tmax=int(cfg['T_max']);lw=int(cfg['len_width']);sw=int(cfg['shift_width']);aux=int(eea.qiskit_paper_aux_size(n,lw,sw,Tmax))
